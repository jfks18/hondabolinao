...
class SecureWebSocketServer {
  ...
  setupHTTPRoutes() {
    this.server.on('request', (req, res) => {
      console.log(`📡 HTTP ${req.method} ${req.url} from origin: ${req.headers.origin || 'none'}`);
      ...
      console.log(`🔍 CORS check: origin=${requestOrigin}, allowed=${JSON.stringify(allowed)}`);

      // Check if wildcard is allowed
      if (allowed.includes('*')) {
        res.setHeader('Access-Control-Allow-Origin', '*');
      } else if (requestOrigin && (allowed.length === 0 || allowed.includes(requestOrigin) || (process.env.NODE_ENV === 'production' && requestOrigin.includes('onrender.com')))) {
      ...
      res.setHeader('Access-Control-Allow-Headers', 'Content-Type, Cache-Control, Authorization, X-Requested-With, Expires, Pragma, If-Modified-Since');
